...
public class DependencyPipe {
  ...
  public int[] createInstances(String file, File featFileName) throws IOException {
  ...
  private final void addLinearFeatures(String type, String[] obsVals, int first, int second,
    ...
    addCorePosFeatures(type + "PT", pLeft, obsVals[first], pLeftRight, pRightLeft, obsVals[second],
            pRight, attachDistance, fv);

  }

  private final void addCorePosFeatures(String prefix, String leftOf1, String one, String rightOf1,
          String leftOf2, String two, String rightOf2, String attachDistance, FeatureVector fv) {
  ...
  /**
   * Add features for two items, each with two observations, e.g. head, head pos, child, and child
   * pos.
   *
   * The use of StringBuilders is not yet as efficient as it could be, but this is a start. (And it
   * abstracts the logic so we can add other features more easily based on other items and
   * observations.)
   **/
  private final void addTwoObsFeatures(String prefix, String item1F1, String item1F2,
  ...
  private void addDiscourseFeatures(DependencyInstance instance, int small, int large,
    ...
    for (int i = 0; i < instance.feats.length; i++) {
      ...
      for (int j = i + 1; j < instance.feats.length; j++) {
        ...
        addTwoObsFeatures("CPF" + i + "_" + j, instance.feats[i][headIndex],
                instance.feats[j][headIndex], instance.feats[i][childIndex],
      ...
      for (int j = 0; j < instance.feats.length; j++) {
        ...
        addTwoObsFeatures("XPF" + i + "_" + j, instance.postags[headIndex],
                instance.feats[i][headIndex], instance.postags[childIndex],
                instance.feats[j][childIndex], attDist, fv);

        addTwoObsFeatures("XCF" + i + "_" + j, instance.cpostags[headIndex],
                instance.feats[i][headIndex], instance.cpostags[childIndex],
                instance.feats[j][childIndex], attDist, fv);
  ...
  public void fillFeatureVectors(DependencyInstance instance, FeatureVector[][][] fvs,
          double[][][] probs, FeatureVector[][][][] nt_fvs, double[][][][] nt_probs,
          Parameters params) {
  ...
  /**
   * Override this method if you have extra features that need to be written to disk. For the basic
   * DependencyPipe, nothing happens.
   *
   */
  ...
  /**
   * Get features for stems the old way. The only way this differs from calling addTwoObsFeatures()
   * is that it checks the lengths of the full lexical items are greater than 5 before adding
   * features.
   *
   */
  private final void addOldMSTStemFeatures(String hLemma, String headP, String cLemma,
          String childP, String attDist, int hL, int cL, FeatureVector fv) {
